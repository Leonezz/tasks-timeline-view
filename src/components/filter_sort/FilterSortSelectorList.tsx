...
function FilterSelectorList({
    ...
    const [filterFiles, setFilterFiles] = useState<string[]>(selectedFilters.files);
    const [filterPriorities, setFilterPriorities] = useState<string[]>(selectedFilters.priorities);
    const [filterStatus, setFilterStatus] = useState<string[]>(selectedFilters.status);
    const [sortCmp, setSortCmp] = useState(selectedFilters.sortCmp);
    const [sortReversed, setSortReversed] = useState<boolean>(selectedFilters.reversed);
    ...
    useEffect(
        ...
        [filterTags, filterFiles, filterPriorities, filterStatus, sortCmp, sortReversed]
    )

    const BuildFilterSelector =
        (label: string, options: string[], selectd: string[], setSelected: (s: string[]) => void) =>
            (options && options.length > 0) ? <FilterSelector key={label}
                label={label}
                options={options}
    ...
    const PriorityFilterSelector = useMemo(
        () => BuildFilterSelector("Priority", options.priorities, filterPriorities, setFilterPriorities),
        [options.priorities, filterPriorities]
    );
    const StatusFilterSelector = useMemo(
        () => BuildFilterSelector("Status", options.status, filterStatus, setFilterStatus),
