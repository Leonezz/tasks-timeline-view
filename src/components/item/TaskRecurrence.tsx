...
const RecurrenceIntervalModeTabs = ({
    ...
    useEffect(() => {
        onValueChange({
            ...
            byweekday:
                repeatBy === Frequency.WEEKLY ||
                (repeatBy === Frequency.MONTHLY &&
                    repeatMonthDayMode === 'weekday')
                    ? repeatWeekdays
                    : null,
            bymonthday:
                repeatBy === Frequency.MONTHLY &&
                repeatMonthDayMode === 'monthday'
                    ? repeatMonthdays
                    : null,
            bysetpos:
                repeatBy === Frequency.MONTHLY &&
                        ...
                        <div className='flex flex-col gap-3'>
                            <div className='flex flex-row gap-2'>
                                ...
                                <Select
                                    ...
                                    selectedKeys={repeatStepos.map((k) =>
                                        k.toString()
                                    )}
                                ...
                                <h2 className='min-w-max self-center'>
                                    Week's
                                </h2>
                                <Select
                                    size='sm'
                                    ...
                                    disallowEmptySelection
                                    selectedKeys={repeatWeekdays.map((k) =>
                                        k.toString()
                                    )}
                                    onSelectionChange={(k) => {
                                        if (k === 'all') return
...
const RecurrencePreview = ({
    ...
    const [previewMonth, setPreviewMonth] = useState(moment())
    ...
    const tileContent = useCallback(
        (date: Date, view: string) => {
            ...
            const dateStr = moment(date).format(innerDateFormat)
            if (!dates.includes(dateStr)) {
                return null
            }
            return (
                <div className="flex justify-center after:flex after:h-4 after:w-4 after:rounded-full after:bg-success after:content-['']" />
            )
        },
        [dates, rruleOptions]
    )
    ...
    return (
        <Calendar
            ...
            // activeStartDate={previewMonth.startOf('month').toDate()}
            onActiveStartDateChange={({ activeStartDate, view }) => {
                if (view === 'month') {
                    setPreviewMonth(moment(activeStartDate as Date))
                }
            }}
            tileDisabled={(args) => {
